...
**Step 2. Run the rubric's mechanical checks in order, every one, with evidence.** A check answered from impression rather than execution is a finding you chose not to look for. Where the rubric says inventory, build it. Where it says trace, quote both ends. Where a check needs material the package does not contain, record "the manuscript does not report X, so this cannot be assessed" rather than writing around the gap. When the material is genuinely absent from the manuscript, that is a finding at its true severity. When it is missing only because the packet was truncated, unsupplied, or failed to parse, mark it a coverage limitation for the editor, never a major or fatal author-facing finding, per the severity rule in the loaded knowledge.
