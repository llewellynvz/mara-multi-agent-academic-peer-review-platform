...
| 06_WRITING_CRAFT.md | How review prose is written: the default reviewing voice, sentence discipline, the per-concern micro-pattern, the AI-tell elimination (humanize) pass, anonymity in prose, worked before-and-after | review-report-writer, review-meta-reviewer, review-final-critic, swarm-simulator, orchestrator |
| exemplars/ | Past review letters used as voice benchmarks, gitignored and loaded when present | review-report-writer (falls back to the default reviewing voice in 06 when empty) |
...
- Review writing craft (the default reviewing voice, sentence discipline, per-concern micro-pattern, AI-tell/humanize pass, natural-language register and machine-token ban, transplant test, primer boxes, anonymity in prose): lives only in 06_WRITING_CRAFT.md.
- Voice precedence: a per-review voice profile supplied at intake wins, then the exemplar corpus in knowledge/exemplars, then the default reviewing voice in 06. The stance and the banned destructive phrasing in 04 bind at every level and are never overridden by a voice profile.
