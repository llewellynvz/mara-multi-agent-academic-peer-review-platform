...
- **UI-27** Block A is confirm-or-correct. Detected field and subfield, study design, and manuscript type each render as a selected chip the user can accept or replace from a short set. Chips use the unified pill recipe, and the detected value is pre-selected. The chips render the `detected` values and `confirm` questions from `GET /api/reviews/{id}/questions` (API-29) and submit through the batch `POST /api/reviews/{id}/answers` (API-10).
- **UI-28** Block B collects four inputs. Target journal is free text with an explicit "None" option. Review preset is Fast, Balanced, or Thorough, defaulting to the setup choice, each showing a cost delta and a time delta in mono. An optional feedback-focus multi-select reweights emphasis only and can never exclude a review lens. An optional free-text field captures anything the reviewer should know.
- **UI-29** The whole screen is skippable. A "Skip with defaults" control proceeds using the detected values and the tier defaults. No question is mandatory. The skip control calls `POST /api/reviews/{id}/answers` with `useDefaults: true` (API-10), submitting every unanswered question at its default.
